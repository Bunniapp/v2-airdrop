...
export async function calculateTestnetAirdrop(): Promise<void> {
  await getTestnetUsers();
  calculateAirdropAmounts();
}


async function getTestnetUsers(): Promise<void> {
...
function calculateAirdropAmounts(): void {
  users.forEach((user) => {
    let userAridropAmount: bigint = 0n;

    if (user.questCompletions >= 5 && user.questCompletions <= 7) {
      userAridropAmount = 100n * 1_000_000_000_000_000_000n * BigInt(user.questCompletions);
    } else if (user.questCompletions >= 8 && user.questCompletions <= 10) {
      userAridropAmount = 200n * 1_000_000_000_000_000_000n * BigInt(user.questCompletions);
    } else if (user.questCompletions >= 11) {
      userAridropAmount = 600n * 1_000_000_000_000_000_000n * BigInt(user.questCompletions);
    }
...
const BUNNI_V2_SEPOLIA_SUBGRAPH_ENDPOINT: string = `https://subgraph.satsuma-prod.com/${process.env.ALCHEMY_SUBGRAPH_KEY}/bacon-labs/bunni-v2-sepolia/version/v0.3.9/api`
