...
import { Address, checksumAddress, encodeAbiParameters, keccak256 as keccak256_viem, parseAbiParameters } from "viem";
...
async function main(): Promise<void> {
  ...
  const [airdropRecipients, merkleRecipients] = users
    ...
    if (
      user.address !== '0xb5087f95643a9a4069471a28d32c569d9bd57fe4' && // ignore BunniHub
...
function generateLeaf(address: Address, value: string): Buffer {
  const singleHash = keccak256_viem(
    encodeAbiParameters(
        parseAbiParameters('address, uint256'),
        [address as Address, BigInt(value)]
    )
  );
