...
import { decodeEventLog, erc20Abi, parseAbiItem, zeroAddress } from "viem";
export async function calculateVeAirdrop() {
    await Promise.all([
        getVeLitHolders(),
        getLiqLitHolders(),
...
async function getVeLitHolders() {
...
async function getLiqLitHolders() {
...
async function getStakedLiqLitHolders() {
    const client = getPublicClient(mainnet);
...
async function getLiqLitLiquidityProviders() {
...
async function getLiquidityProviders_0x37c80638b02ddd9d487320db1de90e50ec9c4ea3(bunniToken) {
    ...
    getUsers().forEach((user) => {
        if (user.balance_0x37c80638b02ddd9d487320db1de90e50ec9c4ea3 > 0n) {
            user.liqLitAmount += user.balance_0x37c80638b02ddd9d487320db1de90e50ec9c4ea3 * liqLitPerLP / 1000000000000000000n;
        }
    });
}
async function getLiquidityProviders_0xa6ece63a84224213a8a706e33b2073a2373335fa(bunniToken) {
    bunniToken.positions.forEach((_position) => {
        const user = getUser(_position.user.address);
        const lpBalance = parseFloat(_position.balance) + parseFloat(_position.gaugeBalance);
    ...
    users.forEach((user) => {
        if (user.balance_0xa6ece63a84224213a8a706e33b2073a2373335fa > 0n) {
            user.liqLitAmount += user.balance_0xa6ece63a84224213a8a706e33b2073a2373335fa * liqLitPerLP / 1000000000000000000n;
        }
    });
}
async function getLiquidityProviders_0x318ed68a45d0e3ee08c36fa35bfac6f8bdcd4f23(bunniToken) {
    bunniToken.positions.forEach((_position) => {
        const user = getUser(_position.user.address);
        const lpBalance = parseFloat(_position.balance) + parseFloat(_position.gaugeBalance);
    ...
    users.forEach((user) => {
        if (user.balance_0x318ed68a45d0e3ee08c36fa35bfac6f8bdcd4f23 > 0n) {
            user.liqLitAmount += user.balance_0x318ed68a45d0e3ee08c36fa35bfac6f8bdcd4f23 * liqLitPerLP / 1000000000000000000n;
        }
    });
}
async function getLitPerBalancerPoolToken() {
    const client = getPublicClient(mainnet);
...
async function calculateAidropAmounts() {
...
const BUNNI_V1_MAINNET_SUBGRAPH_ENDPOINT = `https://gateway.thegraph.com/api/${process.env.GRAPH_SUBGRAPH_KEY}/subgraphs/id/6EcVGBGhW8U6B9nd9uT1n5iDRMtH2evoJeaY5BXBCniM`;
const veLitQuery = gql `
  query veLitQuery (
    $skip: Int = 0,
    $first: Int = 1000
  ) {
...
`;
const liqLitLiquidityProviderQuery = gql `
