...
export async function calculateLiqAirdrop() {
    await Promise.all([
        getLiqHolders(),
        getLockedLiqHolders(),
...
async function getLockedLiqHolders() {
    const client = getPublicClient(mainnet);
...
async function getLiqLiquidityProviders() {
    ...
    users.forEach((user) => {
        if (user.liqLPBalance > 0n) {
            user.liqBalance += user.liqLPBalance * liqPerLP / 1000000000000000000n;
        }
    });
}
async function blacklistLiqHolders() {
...
function teamAndVotersVestingAdjustments() {
...
function calculateAirdropAmounts() {
    const airdropAmount = 10000000n * 1000000000000000000n;
...
const liqLiquidityProviderQuery = gql `
